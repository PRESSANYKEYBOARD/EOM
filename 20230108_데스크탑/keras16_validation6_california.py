...
from sklearn.datasets import fetch_california_housing                               # sklearn에서 제공하는 fetch_california_housing 가져오기
...
model.add(Dense(16, input_dim=8, activation='relu'))                                # 8열이라서 iuput_dim은 8       # x열의 개수      # relu: 0 이하의 값은 다음 레이어에 전달하지 않습니다. 0 이상의 값은 그대로 출력합니다.
...
model.add(Dense(1, activation='linear'))                                            # output=1                       # y열의 개수
...
#2. 모델구성
model = Sequential()                                                                # 모델은 순차적으로 구성하겠다.
model.add(Dense(5, input_dim=8))                                                    # 8열이라서 iuput_dim은 8       # x열의 개수
model.add(Dense(40))
model.add(Dense(110))
model.add(Dense(180))
model.add(Dense(275))                                                               # Dense값을 임의로 조절
model.add(Dense(320))
model.add(Dense(160))
model.add(Dense(130))
model.add(Dense(80))
...
CPU 결과
RMSE: 0.6874191141606308,    R2: 0.6426310687709671,        val_loss: 0.4725

GPU 결과
RMSE: 0.6794666228634973,    R2: 0.6508517725627856,        val_loss: 0.5097
