...
# print(x_train.shape, x_test.shape)                                    # (8708, 2, 2, 2) (2178, 2, 2, 2)

# 2. 모델구성(순차형)
model = Sequential()
model.add(Conv2D(64, (2,2), input_shape=(2, 2, 2)))
...
model.summary()                                                         # 노드의 총 갯수는 drop을 해도 모두 동일하다.
                                                                        # drop을 해도 평가할 때는 전체 노드는 다 쓴다. / 훈련시에만 훈련시에만 적용된다.
...
mcp = ModelCheckpoint(monitor='val_loss', mode='auto', verbose=1,
                    ...
                    #   filepath=path + 'MCP/keras31_ModelCheckPoint3.hdf5')
                      filepath = filepath + 'k39_05' + date + '_' + filename
)
...
submission.to_csv(path + 'submission_01252001.csv')         # 날짜.시간 01061152 = 1월 6일 11시 52분
