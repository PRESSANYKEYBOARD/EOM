...
#3. 컴파일, 훈련
model.compile(loss='sparse_categorical_crossentropy', optimizer='adam',                # 다중 분류 손실함수를 categorical_crossentropy로 많이 사용한다. / 원 핫 인코딩을 거치게 된 후 사용.
             metrics=['accuracy'])
...
# y_test = np.argmax(y_test, axis=1)                                # 원핫을 안했으므로 여기서 할 필요가 없다.
