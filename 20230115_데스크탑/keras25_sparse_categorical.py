...
#2. 모델구성
model = Sequential()
model.add(Dense(50, activation='relu', input_shape=(4, )))      # 모델을 늘리는 것도 성능에 큰 차이를 줌                     
...
#3. 컴파일, 훈련
model.compile(loss='sparse_categorical_crossentropy', optimizer='adam',       # loss로 sparse_categorical_crossentropy를 사용하면 one-hot encoding 필요 X
              metrics=['accuracy'])
...
# y_test = np.argmax(y_test, axis=1)                               # one_hot encoding을 하지 않았으니 쓸 필요가 없다. / np.argmax, acc 필요 x
