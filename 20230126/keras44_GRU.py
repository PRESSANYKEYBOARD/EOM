...
#2. 모델구성
model = Sequential()
# model.add(SimpleRNN(units=10, activation='relu', input_shape=(3, 1)))                   # RNN은 이렇게 명시를 해줘야 한다. / RNN은 3차원인데 행 무시를 하면 2차원
#                                                                                         # DNN은 2차원 이상, 차원을 올릴때마다 인풋쉐이프 해주면 됨
#                                                                                         # CNN은 4차원
                                                                                        
#                                                                                         # (N, 3, 1) → (batch, timesteps, feature)

# model.add(SimpleRNN(units=64, input_length=3, input_dim=1))                                 # 31번 내용을 이렇게 똑같이 쓸 수 있다.
...
# 4 * (2 * 10 + 100)
# 4 * 120 = 480
# 4 units * ( feature units + bias + units )
# 4 * SimpleRNN = LSTM
...
# 3 * ( 100 + 10 * 1 + 10)
# 3 * 120
# 3 * units * ( feature units + bias + units + ? )
# 360이 나와야 하는데 왜 390이 나오누???
