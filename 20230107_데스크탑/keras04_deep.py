...
#1. 데이터
x = np.array([1,2,3,4,5])                           # 넘파이 형식의 어레이 데이터
y = np.array([1,2,3,5,4])                           # 넘파이 형식의 어레이 데이터
...
model.add(Dense(5))                                     
model.add(Dense(4))                                 # Dense값 임의로 조절
model.add(Dense(3))
model.add(Dense(2))
...
model.fit(x, y, epochs=200)                         # epoch: 훈련을 여러번 시키는것. fit: 훈련시킴. # epoch 너무 많이하면 오히려 성능이 떨어지는 경우가 있기 때문에, 적절한 epoch 수치를 찾아내야 함. # 이번엔 훈련 수치를 200번으로 조정.
