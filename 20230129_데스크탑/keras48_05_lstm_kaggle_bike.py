...
mcp = ModelCheckpoint(monitor='val_loss', mode='auto', verbose=1,
                    ...
                    #   filepath=path + 'MCP/keras31_ModelCheckPoint3.hdf5')
                      filepath = filepath + 'k48_05' + date + '_' + filename
)
...
model.save(path+'keras48_LSTM05_save_model.h5')                      # 가중치 및 모델 세이브
...
# 제출할 놈
test_csv = test_csv.reshape(6493, 8, 1)                     # training이 진행된 모델이 conv2D로 시작되어 input_shape이 4차원으로 이뤄져야 함
                                                            # predict에 대입될 데이터도 만들어놓은 모델을 이용하므로 4차원으로 reshape 해줘야 함
...
submission.to_csv(path + 'submission_01291827.csv')         # 날짜.시간 01061152 = 1월 6일 11시 52분
...
cnn
r2스코어: 0.3071642824412648

lstm
r2스코어: 0.3346274876093378
